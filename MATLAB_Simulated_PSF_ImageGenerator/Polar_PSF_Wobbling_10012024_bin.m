% =========================================================================
% Function Name: Polar_PSF_Wobbling_10012024_bin.m
%
% Description:
...
function [I0_teo,I90_teo,I45_teo,I135_teo]=Polar_PSF_Wobbling_10012024_bin(n_1,n_2,NA,lambda,red_factor,RHO_dipole_deg,ETA_dipole_deg,DELTA_dipole_deg,f1,f2,ccd_px_size,show_fig,binsize)
warning('off')
%% Model the PSF %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:length(N_f)
    ...
    Ex1_BFP = (E_xx.*P_dipole1_x + E_xy.*P_dipole1_y + E_xz.*P_dipole1_z).*exp(1i*psy_depth);
    Ey1_BFP = (E_yx.*P_dipole1_x + E_yy.*P_dipole1_y + E_yz.*P_dipole1_z).*exp(1i*psy_depth);
    Eu1_BFP = (Ex1_BFP + Ey1_BFP)./sqrt(2); %45 deg projection
    Ev1_BFP = (-Ex1_BFP + Ey1_BFP)./sqrt(2); %135 deg projection

    Ex2_BFP = (E_xx.*P_dipole2_x + E_xy.*P_dipole2_y + E_xz.*P_dipole2_z).*exp(1i*psy_depth);
    Ey2_BFP = (E_yx.*P_dipole2_x + E_yy.*P_dipole2_y + E_yz.*P_dipole2_z).*exp(1i*psy_depth);
    Eu2_BFP = (Ex2_BFP + Ey2_BFP)./sqrt(2); %45 deg projection
    Ev2_BFP = (-Ex2_BFP + Ey2_BFP)./sqrt(2); %135 deg projection

    Ex3_BFP = (E_xx.*P_dipole3_x + E_xy.*P_dipole3_y + E_xz.*P_dipole3_z).*exp(1i*psy_depth);
    Ey3_BFP = (E_yx.*P_dipole3_x + E_yy.*P_dipole3_y + E_yz.*P_dipole3_z).*exp(1i*psy_depth);
...
m =  sum(Ix(:,:,2),'all')+ sum(Iy(:,:,2),'all')+ sum(Iu(:,:,2),'all')+ sum(Iv(:,:,2),'all') ; % simplify : dz=0 plane for all. Total 4 channels.
Ix= Ix/m;
Iy= Iy/m;
Iu= Iu/m;
Iv= Iv/m;
...
%% resize with binsize
IxtempT=bin_image(Ix_save(:,:,2),binsize);
IytempT=bin_image(Iy_save(:,:,2),binsize);
IutempT=bin_image(Iu_save(:,:,2),binsize);
IvtempT=bin_image(Iv_save(:,:,2),binsize);
newX=X(1:binsize:end);
newY=Y(1:binsize:end);

Xlims=(newX>-1)&(newX<1);
Ylims=(newY>-1)&(newY<1);
factor_correction_0=sum(Ix_save(:,:,2),"all")/sum(IxtempT(Ylims,Xlims),"all");
factor_correction_90=sum(Iy_save(:,:,2),"all")/sum(IytempT(Ylims,Xlims),"all");
factor_correction_45=sum(Iu_save(:,:,2),"all")/sum(IutempT(Ylims,Xlims),"all");
factor_correction_135=sum(Iv_save(:,:,2),"all")/sum(IvtempT(Ylims,Xlims),"all");
